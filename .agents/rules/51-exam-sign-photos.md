...
- `examSignImages.ts` (generated by the workflow) is a list of **candidates**.
  ...
  its own.
- `GeneralExamSigns.tsx` shows a picture only when its id is in
  `REVIEWED_SIGNS`. (It used to mean "a bundled asset exists", which happened
  to work and said nothing about whether anybody had looked.)
...
## Four sources, and the query shape that made three of them silent

Commons alone was the limit for months, and the fetcher reported "nothing
acceptable exists" when it meant "nothing acceptable is here". The chain is
now, in order:

1. `commonsFiles` — named files, tried first, licence still checked.
2. **Open-i** (`openi.nlm.nih.gov`) — NLM's index over PubMed Central's
   open-access subset, and the only source that can filter to PHOTOGRAPHS
   (`it=ph`) rather than charts.
3. **Commons categories** — membership is a human's judgement that the file
   depicts the thing. Nobody files a wrestler under Category:Anemia.
4. Commons `intitle:` search, then free text, then **Openverse**, then
   **Europe PMC** figures from CC BY case reports.

**Terms must be two words, not three.** Open-i, Europe PMC and Openverse each
read a term as a *phrase*. `"conjunctival pallor anaemia"` is a phrase no
article contains, so all three returned zero for every sign and only Commons
ever answered — which is why the blanks looked like a corpus problem. This bug
was found on pallor and was still present in all five remaining signs
afterwards. Write what a doctor writes: `conjunctival pallor`, `cachexia`,
`moon facies`.

**Europe PMC is capped at 8 articles and 4 figures each.** It fetches the full
text of every article it looks at, so a probe of all nineteen signs ran for
eighteen minutes without finishing and had to be cancelled.

## Probe before fetching

`workflow_dispatch` with `probe: true` prints what every source offers for the
signs named in `only` (comma separated) and **commits nothing**. Use it. Five
rounds of blind fetch-and-look produced, for one word, an engraving, a
wrestler, a beer parlour, a starling and a Loa loa worm — each of which had to
be downloaded and opened to be rejected, because the run only ever printed the
winner.

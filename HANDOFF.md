...
### 2026-09-26 — Shared daily Home questions (v24 follow-up)

- Product source commit `fe03a978bfa2f4c88cd8f850fffc46c08bc4d5bb` places MCQ of the day and Picture of the day inside the large Welcome carousel, followed by Resume, Progress, Study time, Attendance. The small Resume tile was replaced by Bank; the childish "Swipe for more" caption was removed. `homeResume.ts` now validates real YearKey values (`second-year`, etc.), so saved questions actually reopen.
- `mobile/src/lib/dailyStudy.ts` caches each card and answer by local date and selected year. The new `daily-study-card` Supabase Edge Function returns a shared card for year/kind/date. Private `daily_study_cards` holds 42 reusable slots per year/kind; a slot is generated only once and later repeats without Gemini calls. At most 336 unique slots for all four years and both kinds. Unique row claims avoid duplicate calls when users open the same slot concurrently. Picture questions use approved, reviewed diagrams in the matching year. Gemini model is `gemini-3.1-flash-lite`, with image input for picture cards. Server key stays in the Edge Function; table RLS is on with no client grant. Deployed function version 2, `verify_jwt=true`; migration `create_shared_daily_study_pool` applied. Live check generated Second Year MCQ and picture and stored them ready.
- `mobile/preview/customization-visual.mjs` seeds daily cards and a saved question to capture actual React Native Web Home components at phone size. Workflow `customization-visual.yml` runs a live Edge check first, then captures MCQ, picture, Resume, other pages and quick actions. Preview fixtures are only for screenshots; the installed app calls the shared Edge Function.
- The daily prompt is grounded in a reviewed diagram row from the chosen year. It is not individually generated from each person's ticked or bookmarked question; shared by year is deliberate to avoid one Gemini call per user. It rotates through 42 slots (six weeks) and repeats after the pool is filled. Device-local answers are independent.
- CI visual run from the first commit passed, then a follow-up corrected the blank image capture and clipped Bank label. Check the latest visual run and release/internal/debug runs from `fe03a978...`, inspect screenshots and hand over the new build links. VersionCode remains 24; no Play Console upload was performed.

### Final daily Home visual and storage check

- Latest native source commit `d4e0dd2d3fedfa1b9fde7553dcae4e867660ebd4`; server-only pool distinctness commit `815da7da063fd892cc6741030edc25726f8704ce`; deployed `daily-study-card` is version 3. The server now chooses unique diagram positions for each of the 42 slots even in Final Year. The daily picture opens an opaque full-screen diagram with Close.
- Final phone-size visual workflow `36213138445` passed, including actual MCQ, picture, enlarged image, saved Resume, and quick-action screenshots. Reviewed shareable images are committed under `screenshots/home-daily-v24/` at screenshot commit `7458277e6a50a43e4f7bfe6c93183e625d369e92` (four compressed JPGs; the artifact also has full-resolution PNGs).
- Supabase live count: `daily_study_cards` has 2 ready Second Year cards, 32 KiB table and index footprint, 1,491 bytes of card JSON. Existing Storage has 407 objects = 290,650,187 bytes (~277.2 MiB): diagrams 249,208,275 bytes, textbooks 41,441,912 bytes. Database is 66,555,027 bytes (~63.5 MiB), measured separately. The daily feature stores only card text and existing diagram URLs; it uploads no picture copies. Full 336-slot pool is expected to remain roughly under 1 MiB, depending on database index overhead. The anonymous and authenticated roles have no SELECT grant on the cache table; service_role does.
- `app_releases` v24 cloud row was updated to match the new daily cards and other v24 Home changes. Play Console release name is `24 (0.0.0.24)`; release notes are drafted at 382 characters under `<en-US>`.
- Final Android runs from `d4e0dd2d...`: release `36213138345`, internal `36213138347`, debug `36213138460`. All three passed typecheck, lint and checks and are compiling at the time of this note. Verify their final conclusions, tags, and assets before handing them out. Do not point to earlier `release-530` etc. as containing daily cards. No Play Console upload was done.

### Final verified v24 daily-card artifacts

- Final Android runs **all succeeded**: release `36213138345`, internal `36213138347`, debug `36213138460`. Typecheck, lint and required checks passed before packaging. All three tags resolve to `49e52ed9490ecd181b893cfc4252831af36d6b60`; compared with source run commit `d4e0dd2d3fedfa1b9fde7553dcae4e867660ebd4`, the tag adds only handoff text and reviewed screenshots.
- Signed Play AAB: https://github.com/Sabharivarshan111/gmck/releases/download/release-536/app-release.aab (80,115,807 bytes). Release APK: https://github.com/Sabharivarshan111/gmck/releases/download/release-536/app-release.apk (101,248,380 bytes). These retain normal Google sign-in and live ads.
- Internal APK: https://github.com/Sabharivarshan111/gmck/releases/download/internal-337/app-internal.apk (108,883,660 bytes), normal Google sign-in without ads. Debug preview APK: https://github.com/Sabharivarshan111/gmck/releases/download/debug-343/app-preview.apk (108,883,676 bytes), debug-only Google sign-in bypass and no ads.
- VersionCode is 24 / versionName 0.0.0.24. User said Play currently has 23. AAB was not uploaded to Play Console in this work. Release name to paste: `24 (0.0.0.24)`. Release notes are in the ChatGPT handoff below or the owner's conversation, under 500 characters per English language.

### 2026-09-26 compact daily flashcards after owner review

- Owner rejected the tall daily MCQ/picture slide. Product source `30e5b00f4f78811a6250b172349825e55e2a0341` fixes the Home carousel page height at 184 dp so Welcome, daily front/answer, Resume, progress, study time and attendance have the same compact card boundary. The daily front shows a small tappable diagram, question, two-line options and a Reveal answer button. Revealed answer/explanation remain in the card; full text and enlarged picture open on tap. Existing local card answers still read correctly, while a reveal with no selection is saved as `revealed` without falsely counting a correct selection.
- The preceding `46420e7ec2b6fa91f665be1c2594f73e3d5174c1` prevents duplicate daily requests during fast slide changes and ignores responses from a previous year/date. Local `dailyStudy.ts` reads, writes and saves against the same request date.
- Phone viewport visual run `36226028426` passed: screenshot set includes `home-daily-mcq.png`, `home-daily-mcq-selected.png`, `home-daily-mcq-answer.png`, `home-daily-picture.png`, `home-daily-picture-answer.png` and the expanded picture. It asserts the Next arrow stays at the same Y on Welcome, MCQ front/back and Picture front/back. Screenshots are from the actual Home component rendered via React Native Web at 412×915, not a native Android device capture. Artifact ID `10900163262`.
- Final compact build runs from `30e5b00f...`: release `36226028404`, internal `36226028480`, debug `36226028434`; verify final success and tags before handing out links. These supersede `release-536`, `internal-337`, `debug-343` because those contain the tall daily question card. The cloud shared pool is unchanged; no added Gemini call or image upload is required for the compact design. No Play Console upload was performed.

#### Verified compact v24 files

- All three final runs passed and all tags point exactly to compact source `30e5b00f4f78811a6250b172349825e55e2a0341`.
- Play release `release-539`: signed [AAB](https://github.com/Sabharivarshan111/gmck/releases/download/release-539/app-release.aab) (80,117,529 bytes), signed [APK](https://github.com/Sabharivarshan111/gmck/releases/download/release-539/app-release.apk) (101,251,880 bytes), normal Google sign-in and live ads.

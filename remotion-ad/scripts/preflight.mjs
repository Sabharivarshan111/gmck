...
/* ------------------------------------------------------------------------
 * A plate is the real picture or it is not a plate.
 *
 * `fetch-plates` never writes a stand-in — it fails instead — but the staging
 * directory the preview harness reads from is not wiped between runs, and it
 * had collected three 4,843-byte PNGs carrying `.jpg` names from a session
 * where the download had failed. `plate-tca-cycle.jpg` therefore *existed*,
 * so the single-note screen asked for it, got a grey ramp, and photographed
 * it — inside a card headed "High-Yield Visual Exam Diagram". That screen is
 * what every note shot in every ad is built on.
 *
 * The size floor above cannot see it: a stand-in is comfortably over 4KB. So
 * this reads the first three bytes. A JPEG starts FF D8 FF; anything else
 * under a plate's filename is something else wearing it.
 * --------------------------------------------------------------------- */
{
  const dir = path.join(root, 'public', 'app_screens');
  for (const name of await fs.readdir(dir).catch(() => [])) {
    if (!name.startsWith('plate-') || !name.endsWith('.jpg')) continue;
    const head = Buffer.alloc(3);
    const fh = await fs.open(path.join(dir, name), 'r');
    await fh.read(head, 0, 3, 0);
    await fh.close();
    if (!(head[0] === 0xff && head[1] === 0xd8 && head[2] === 0xff)) {
      problems.push(
        `app_screens/${name} is not a JPEG. A plate under a plate's filename ` +
          'that is not the picture is a stand-in, and it photographs as a ' +
          'broken diagram inside an ad. Run `npm run plates`.',
      );
    }
  }
}
...
/* ------------------------------------------------------------------------
 * The shot boundaries have to describe the lines the scripts hold today.
 *
 * `src/dynamicScriptTimings.ts` is where a long-form ad's shot lengths come
 * from, measured off the recordings. `Root.tsx` reads it. It is generated by
 * `measure-audio`, and the committed copy is deliberately empty — but nothing
 * checked that, and it sat in the repo holding 90 rows whose `vo` was a line
 * the scripts no longer contained: every shot of all three long-form ads,
 * left behind by a rewrite.
 *
 * A render against those does not fail. It lays new recordings on boundaries
 * measured from old ones, so one line that grew pushes every later shot out of
 * step and the voice runs under the next picture. That is the overlapping
 * voice, arriving by a different road from the one already fixed.
 * --------------------------------------------------------------------- */
{
  const { DYNAMIC_SCRIPT_TIMINGS } = await import(
    pathToFileURL(path.join(root, 'src', 'dynamicScriptTimings.ts')).href
  ...
  for (const script of scripts) {
    ...
    for (const shot of script.shots) {
      const row = table.shots.find((r) => r.n === shot.n);
      if (row && shot.vo && row.vo !== shot.vo) {
        stale += 1;
        if (!example) example = `${script.id} shot ${shot.n}: measured "${row.vo}"`;
      }
    }
  }
  if (stale > 0) {
    problems.push(
      `src/dynamicScriptTimings.ts has ${stale} row(s) describing a line the ` +
        `script no longer contains (${example}). Those boundaries were measured ` +
        'from different recordings, so every later shot in that ad starts at ' +
        'the wrong frame. Run `npm run voice`.',
    );
  }
}
...
/*
 * ---- The end card's badge slot, generated rather than hand-flipped -------
 *
 * A Remotion composition runs in a browser and cannot ask whether a file
 * exists — a fetch that 404s mid-render is a hole in a finished frame. So the
 * answer is baked here, where the filesystem is visible, and `EndCard` reads
 * the constant.
 *
 * The point is that dropping the official artwork at
 * `public/google-play-badge.png` is the WHOLE job: no code change, no flag to
 * remember. It is generated on every preflight, which every render runs first.
 */
{
  const badgePath = path.join(root, 'public', 'google-play-badge.png');
  const present = await fs.stat(badgePath).then((s) => s.size > 1000).catch(() => false);
  const file = path.join(root, 'src', 'components', 'playBadge.ts');
  const body = `/**
 ...
 */
export const HAS_PLAY_BADGE = ${present};
`;

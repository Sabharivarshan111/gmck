...
export interface Shot {
  ...
  screen: string | null;
  /**
   * How the camera moves on the device in this shot.
   *
   * Optional, because it belongs to the `device` look and only to that one:
   * the `prompt` and `keynote` ads have no device flying through 3D space for
   * a camera to be moving on. Giving those a camera value would be inventing a
   * move nothing performs, which is worse than leaving it out.
   *
   * `check:ad-looks` requires it on every shot of a device ad, so it cannot be
   * dropped from the ads that DO move.
   */
  camera?: CameraMove;
  ...
  kicker?: string;
  /**
   * Type this shot's `text` out character by character, with a caret.
   *
   * Only the `prompt` look uses it, and only on the shots that ARE a question.
   * A whole ad of typing is a whole ad of waiting, so the question is typed
   * and the answer simply arrives.
   */
  typed?: boolean;
  /**
...
export interface AdScript {
  ...
  /**
   * Which visual language renders this script.
   *
   * `device` is everything that existed before: a titanium phone moving
   * through 3D space with a headline in the platform-safe band. The two
   * others are the ads built against
   * `Tejashmakwana/astra-chatgpt-hyperframes`, at the owner's request, and
   * they are deliberately NOT that film with different words — a 3D device
   * shot and a flat typographic one are two different kinds of ad, and running
   * both through one renderer is how they would become one.
   *
   * * `prompt` — a light ground, a question being typed, the app answering.
   * * `keynote` — a dark ground, swept-gradient title cards against
   *   full-bleed screens.
   *
   * Absent means `device`, so nothing that exists today changes.
   */
  look?: 'device' | 'prompt' | 'keynote';
  /**
   * Authored for the HyperFrames films, and not registered as a Remotion
   * composition.
   *
   * It is still in `ALL_SCRIPTS`, which is the point: `check:ad-truth` holds
   * its lines to the screenshots under them, `preflight` holds it to sixty
   * seconds of speech, and the no-counting and no-"plate" rules apply. What it
   * skips is `Root.tsx`, because a script written for a flat HTML film cut at
   * two and a half seconds a shot would render through the device renderer as
   * a worse version of an ad that already exists.
   */
  hyperOnly?: boolean;
  /**

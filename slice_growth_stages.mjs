...
/**
 * Locked Ground Baseline Slicer:
 * Extracts each tree sprite with dynamic valley bounding, perfectly centers horizontally (X=50%),
 * and locks the bottom soil/earth baseline to a single consistent Y line across all 24 stages!
 */
export function sliceWithLockedBaseline(imagePath, speciesKey, outputBaseDir, canvasSize = 256) {
  ...
  // Consistent ground baseline across the entire species sequence (82% from top)
  const groundBaselineY = Math.round(canvasSize * 0.84);
  const maxAvailableHeight = groundBaselineY - Math.round(canvasSize * 0.06); // ~200px available for full tree height

  let stageNum = 1;
  rowIntervals.forEach((row) => {
    // 2. Dynamic Column Valleys
    ...
    // 3. Extract each cell with locked baseline
    for (let c = 0; c < 6; c++) {
      ...
      if (count > 20 && maxX >= minX && maxY >= minY) {
        ...
        // Proportional scale:
        // Fits comfortably within maxAvailableHeight without exceeding canvas width
        let scale = Math.min((canvasSize * 0.88) / itemW, maxAvailableHeight / itemH);
        
        // For early small seeds (Stage 1..3), give them slight magnification so they are prominent, while keeping their base anchored!
        if (stageNum <= 3 && itemH < maxAvailableHeight * 0.4) {
          scale = Math.min((canvasSize * 0.65) / itemW, (maxAvailableHeight * 0.5) / itemH);
        } else if (stageNum <= 6 && itemH < maxAvailableHeight * 0.55) {
          scale = Math.min((canvasSize * 0.75) / itemW, (maxAvailableHeight * 0.65) / itemH);
        }

        const renderW = Math.round(itemW * scale);
        const renderH = Math.round(itemH * scale);

        // Center horizontally & lock bottom soil to groundBaselineY
        const offsetX = Math.round((canvasSize - renderW) / 2);
        const offsetY = Math.round(groundBaselineY - renderH);
  ...
  console.log(`🌱 Baseline-Locked & Centered ${speciesKey} (24 stages)`);
}
...
for (const tm of TREE_MAPPINGS) {
  ...
  if (fs.existsSync(fullPath)) {
    sliceWithLockedBaseline(fullPath, tm.species, outputTreesDir, 256);
  }
}
console.log("\n🌳 ALL 16 SPECIES (384 FRAMES) LOCKED TO GROUND BASELINE!");

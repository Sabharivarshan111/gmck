...
/*
 * The links live in `@/lib/whatsappGroups`, shared with the native app.
 *
 * Two hard-coded URLs used to live here and they were LABELLED WRONG: the one
 * called `WHATSAPP_GROUP_URL_SECOND_YEAR` is the 3rd-year 2024-batch group and
 * the one shown to third years is the final-year question-bank group. First
 * years and final years had no link at all — every one of them was handed the
 * third year's. The names of the groups themselves are the evidence, and they
 * are in the shared file now so neither app can drift from the other.
 */
...
export function useGroupYear(): GroupYear {
  ...
  return y === "first" || y === "second" || y === "third" || y === "final"
    ? y
    : "third";
}
...
export function WhatsAppGroupCard({ note, year }: { note?: string; year?: GroupYear }) {
  ...
  const resolved = year ?? detected;
  const groups = groupsForYear(resolved);
  const label = YEAR_LABEL[resolved];
  return (
    <div className="rounded-2xl border border-border/60 bg-card p-4 space-y-3">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="font-bold text-sm">
            {groups.length > 1
              ? `WhatsApp groups for ${label}`
              : `WhatsApp group for ${label}`}
          </p>
          <p className="text-xs text-muted-foreground leading-relaxed mt-1">
            {note ??
              (groups.length > 1
                ? `${label} has ${groups.length} groups. Open whichever is yours — you can join both.`
                : `Join our WhatsApp group for ${label} study materials, notes and exam updates.`)}
          </p>
        </div>
      </div>
      {/*
        One button per group, named for the group.

        Final year genuinely has two — the batch sitting the exam, and the 2023
        question-bank group — and picking one for the reader sends half of them
        to the wrong room. Every other year has exactly one and this renders a
        single button, which is what it always looked like.
      */}
      {groups.map((group) => (
        <a
          key={group.code}
        ...
        >
          {groups.length > 1 ? group.name : "Tap here to join our WhatsApp group"}
        </a>
      ))}
      <PlayStoreWarning />
    </div>
...
/**
 * Slim home-screen row — the group that matches the selected year.
 *
 * A year with several groups gets one row each rather than a chooser: this is
 * already a compact list, and a row that opens a dialog to reveal two more rows
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-semibold leading-tight">
              {groups.length > 1 ? group.name : "Join our WhatsApp community"}
            </span>

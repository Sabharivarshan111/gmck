...
function ImportPanel({
  ...
  const [staged, setStaged] = useState<{ fileName: string; pkg: ImportedApkg } | null>(null);
  const [chosen, setChosen] = useState<Set<string>>(new Set());

  const summaries = useMemo(
    () => (staged ? deckSummary(staged.pkg.collection) : []),
    [staged]
  );
  ...
  const onFile = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStaged({ fileName: file.name, pkg });
  ...
  const importChosen = useCallback(async () => {
    ...
    setError(null);
    try {
      const deck = await importPackage(staged.pkg, staged.fileName, { decks: chosen });
      setStaged(null);
      setChosen(new Set());
      onDecks(loadImportedDecks());
      onStudy(deck);
    } catch (e) {
      setError((e as Error).message || "That package could not be imported.");
    } finally {
      setBusy(null);
    }
  ...
  return (
    <div className="rounded-xl border bg-card p-4 space-y-3">
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          Open an .apkg or .colpkg from Anki, AnkiWeb or Downloads. It is read in this
          browser and stays here — nothing is uploaded.
        </p>
      </div>
      ...
      {!staged ? (
        <label className="block">
          <input
            type="file"
            accept=".apkg,.colpkg,application/zip,application/octet-stream"
            className="sr-only"
            disabled={busy !== null}
            onChange={onFile}
          ...
          >
            ...
            {busy ?? "Choose an .apkg or .colpkg file"}
          </span>
        </label>
      ) : (
        <div className="space-y-3 rounded-xl border bg-muted/20 p-3">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                Choose the sub-decks to bring into Orbit.
              </p>
            </div>
            <button
              type="button"
